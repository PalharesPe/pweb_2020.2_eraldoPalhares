...
public class ProdController {
	...
	public ModelAndView listarProdutos() {
		List<OutroProduto> lista = produtoRepo.findAll();
	...
	public ModelAndView formCadastrarProdutos() {
		...
		modelAndView.addObject(new OutroProduto());
		return modelAndView;
	}
	@PostMapping("/cadastrarProdutos")
	public String cadastrarProdutos(OutroProduto p) {
	...
	public ModelAndView formEditarProduto(@PathVariable("id") long id) {
		OutroProduto produto = produtoRepo.findById(id).orElseThrow(() -> new IllegalArgumentException("ID inválido:" + id));
		ModelAndView modelAndView = new ModelAndView("editarProdutos");
		modelAndView.addObject(produto);
		return modelAndView;
	}
	@PostMapping("/editarprod/{id}")
	public ModelAndView editarProdutos(@PathVariable("id") long id, OutroProduto p) {
		this.produtoRepo.save(p);
	...
	public ModelAndView removerProduto(@PathVariable("id") long id) {
		OutroProduto aRemover = produtoRepo.findById(id).orElseThrow(() -> new IllegalArgumentException("ID inválido:" + id));
